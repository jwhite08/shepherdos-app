...
export default function AccessControl({ t }) {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([adminApi.listUsers(), ministriesApi.list()])
      .then(([u, m]) => { setUsers(u.users); setAll(m.ministries); setError(""); })
  ...
  if (error)   return <><PageHeader t={t} title="Access Control" /><ErrorMsg t={t} message={error} /></>;
    ...
    <div>
      ...
      <div style={{ padding: "22px 32px" }}>
        <div style={{ background: t.white, borderRadius: 12, border: `1px solid ${t.border}`, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 2.4fr 90px", padding: "12px 20px", borderBottom: `1px solid ${t.border}`, background: t.surface, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: t.textMuted }}>
            <span>User</span><span>Role</span><span>Ministry Access</span><span />
          </div>

          {users.map(u => (
            <div key={u.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 2.4fr 90px", padding: "14px 20px", borderBottom: `1px solid ${t.border}`, alignItems: "center", opacity: u.isActive ? 1 : 0.5 }}>
              <div>
...
function EditAccessModal({ t, user, allMinistries, isSelf, currentUserRole, onClose, onSaved }) {
  ...
  const [canViewFinance, setFinance]      = useState(user.canViewFinance);
  ...
  return (
    <Modal t={t} title={`Access — ${user.firstName} ${user.lastName}`} onClose={onClose} width={620}>
      ...
      {isSelf && !isGlobal && (
        <div style={{ background: "#FDECEC", color: t.danger, borderRadius: 9, padding: "10px 13px", fontSize: 12, marginBottom: 16 }}>
